  ...
  try {
    ...
    if (previousSiblingTagName != "script") {
          ...
          if (classHasSpace) {
            ...
            if (temp.startsWith("//")) {
              if (
                ...
                evaluateXPathExpression(temp).singleNodeValue.attributes
                  .letxxpath != undefined
              ) {
                XPATHDATA.push([8, "Following sibling XPath", temp]);
              } else {
                let t = addIndexToXpath(
      ...
      if (temp == null || preSib.innerText.length > 1) {
        ...
        if (
          c == 1 &&
          evaluateXPathExpression(temp1).singleNodeValue.attributes.letxxpath !=
            undefined
        ) {
        ...
        if (checkTem.singleNodeValue.hasAttribute("letxxpath")) {
          ...
          if (c >= 1) {
            try {
              ...
              if (
                checkTem.singleNodeValue.attributes.letxxpath.value === "letX"
              ) {
