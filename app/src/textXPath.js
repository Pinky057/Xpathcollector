  ...
  if (element.textContent.length > 0) {
    ...
    if (tagName === "a") {
      ...
      if (checkReturn && gotPartial == false) {
        ...
        gotPartial == false &&
        getNumberOfXPath(`//a[text()='${link.trim()}']`) == 1
      ) {
        XPATHDATA.push([0, "Link Text", link.trim()]);
        XPATHDATA.push([
          0,
    ...
    if (tagName != "select" && tagName != "a") {
      ...
      if (hasBr) {
        ...
        for (let i = 0; i < otherChild.length; i++) {
          if (
            otherChild[i].textContent.length > 1 &&
            otherChild[i].textContent.match(/\w/g)
          ) {
    ...
    if (textBasedXpath != null) {
      if (textBasedXpath.startsWith("//") || textBasedXpath.startsWith("(")) {
        ...
        if (len > 2) {
          ...
          let temp = textBasedXpath.replace(
            textBasedXpath.charAt(firstIndex),
            `"`
          );
